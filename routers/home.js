...
homeRouter.get("/emailPedido", async (req, res) => {
  try {
...
homeRouter.get("/mensaje", async (req, res) => {
  try {
    console.log("Mensaje de Pedido en Proceso");
    /*const numUsuario = req.user.phone;
    await client.messages.create({
      body: "Su pedido ha sido recibido y se encuentra en proceso",
      from: "+14582434892",
...
homeRouter.get("/whatsapp", async (req, res) => {
  try {
    console.log("Whatsapp de Pedido en Proceso");
    /* const name = req.user.name;
    const email = req.user.username;
    await client.messages.create({
